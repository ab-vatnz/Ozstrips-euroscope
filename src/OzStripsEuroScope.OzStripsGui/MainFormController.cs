...
public class MainFormController : IDisposable, IStripsWindow
{
    ...
    /// <summary>
    /// Gets the currently selected aerodrome.
    /// </summary>
    public string CurrentAerodrome => _bayManager?.AerodromeName ?? string.Empty;

    /// <summary>
    /// Gets the currently selected OzStrips server.
    /// </summary>
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
    ...
    /// <summary>
    /// Adds aerodromes discovered from EuroScope to the quick-switch list.
    /// </summary>
    /// <param name="value">The aerodromes to merge.</param>
    public void MergeCustomAerodromeList(IEnumerable<string> value)
    {
        var merged = _mainForm.AerodromeManager.ManuallySetAerodromes.ToList();

        foreach (var aerodrome in value.Select(x => x.ToUpper(CultureInfo.InvariantCulture)).Where(x => x.Length == 4))
        {
            if (!merged.Contains(aerodrome))
            {
                merged.Add(aerodrome);
            }
        }

        SetCustomAerodromeList(merged);
    }

    /// <summary>
    /// Sets the server type from the EuroScope bridge.
    /// </summary>
    /// <param name="type">The server type.</param>
    public void SetServerType(ConnectionMetadataDTO.Servers type)
    {
        if (_socketConn.Server != type)
        {
            _socketConn.SetServerType(type);
        }
    }
