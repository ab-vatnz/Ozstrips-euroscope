...
public sealed class Strip : IDisposable
{
    ...
    public string CFL
    {
        get => FDR.CFLAssigned ? FDR.CFLString : string.Empty;
    ...
    /// <summary>
    /// Gets the filed requested flight level, regardless of display state.
    /// </summary>
    public string FiledRFL => FDR.RFL <= 0 ? string.Empty : (FDR.RFL / 100).ToString(CultureInfo.InvariantCulture).PadLeft(3, '0');

    /// <summary>
    /// Gets the squawk display text.
    /// </summary>
    public string DisplaySSR => FormatSSR(FDR.AssignedSSRCode);
    ...
    public string RWY
    {
        get
        {
            ...
            if (StripType == StripType.ARRIVAL && !string.IsNullOrWhiteSpace(routeStarRunway))
            {
            ...
            {
                if (FDR.DepartureRunwayAssigned)
                {
                    return FDR.DepartureRunway.Name;
                }
            }

            if (StripType == StripType.ARRIVAL && FDR.STAR is not null)
            {
                var starRunway = SidRunwayFromRaw(FDR.STAR.Name).Runway;
                if (!string.IsNullOrWhiteSpace(starRunway))
                {
                    return starRunway;
                }
            }
        ...
        set
        {
            if (DefaultStripType == StripType.DEPARTURE || DefaultStripType == StripType.LOCAL)
            {
                ...
                var runway = runways.FirstOrDefault(x => string.Equals(x.Name, value, StringComparison.OrdinalIgnoreCase));
                if (runway == null && !string.IsNullOrWhiteSpace(value))
                {
                    runway = Airspace2.GetOrCreateRunway(aerodrome, value.Trim().ToUpperInvariant());
                }
            ...
            else if (DefaultStripType == StripType.ARRIVAL)
            {
                var aerodrome = FDR.DesAirport;
                var runways = Airspace2.GetRunways(aerodrome);
                var runway = runways.FirstOrDefault(x => string.Equals(x.Name, value, StringComparison.OrdinalIgnoreCase));
                if (runway == null && !string.IsNullOrWhiteSpace(value))
                {
                ...
                FDR.ArrivalRunway = runway;
                if ((Network.Me.IsRealATC || MainForm.IsDebug) && FDR.STAR is not null)
                {
                    SetSTAR(FDR, new SIDSTAR
                    {
                        Name = FDR.STAR.Name,
                        Runway = runway?.Name ?? string.Empty,
                    });
                }
            }
        }
    ...
    public string SID
    {
        get
        {
            if (StripType == StripType.ARRIVAL)
            {
                ...
                return FDR.STAR is not null ? SidRunwayFromRaw(FDR.STAR.Name).Sid : string.Empty;
            }
        ...
        set
        {
            if (DefaultStripType == StripType.ARRIVAL)
            {
                var starFound = false;
                var possibleStars = (FDR.ArrivalRunway?.STARs ?? [])
                    .Concat(Airspace2.GetRunways(FDR.DesAirport).SelectMany(x => x.STARs))
                    .GroupBy(x => x.sidStar.Name, StringComparer.OrdinalIgnoreCase)
                    .Select(x => x.First());

                foreach (var possibleSTAR in possibleStars)
                {
                    if (string.Equals(possibleSTAR.sidStar.Name, value, StringComparison.OrdinalIgnoreCase))
                    {
                        if (Network.Me.IsRealATC || MainForm.IsDebug)
                        {
                            SetSTAR(FDR, possibleSTAR.sidStar);
                        }
                ...
                if (!starFound && (Network.Me.IsRealATC || MainForm.IsDebug))
                {
                    SetSTAR(FDR, string.IsNullOrWhiteSpace(value) ? null : new SIDSTAR
                    {
                        Name = value.Trim(),
                        Runway = FDR.ArrivalRunway?.Name ?? string.Empty,
                    });
                }
            ...
            var found = false;
            var possibleSids = (FDR.DepartureRunway?.SIDs ?? [])
                .Concat(Airspace2.GetRunways(FDR.DepAirport).SelectMany(x => x.SIDs))
                .GroupBy(x => x.sidStar.Name, StringComparer.OrdinalIgnoreCase)
                .Select(x => x.First());

            foreach (var possibleSID in possibleSids)
            {
                if (string.Equals(possibleSID.sidStar.Name, value, StringComparison.OrdinalIgnoreCase))
                {
                    if (Network.Me.IsRealATC || MainForm.IsDebug)
                    {
            ...
            if (!found)
            {
                if (Network.Me.IsRealATC || MainForm.IsDebug)
                {
                    SetSID(FDR, string.IsNullOrWhiteSpace(value) ? null : new SIDSTAR
                    {
                        Name = value.Trim(),
    ...
    public static string FormatSIDForDisplay(string sid)
    {
    ...
    public void FillStrip()
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(result.CFL) &&
                int.TryParse(FiledRFL, NumberStyles.Integer, CultureInfo.InvariantCulture, out var filedRfl) &&
                int.TryParse(result.CFL, NumberStyles.Integer, CultureInfo.InvariantCulture, out var resultCfl))
            {
                CFL = filedRfl < resultCfl ? FiledRFL : result.CFL;
            }
    ...
    public bool DetermineSCValidity()
    {
        ...
        if (distance > 50 && DefaultStripType == StripType.DEPARTURE)
        {
            return false;
        }
    ...
    {
        MMI.OpenFPWindow(FDR, position ?? Cursor.Position);
    }
